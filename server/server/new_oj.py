...
def render_board(pid):
	...
	if len(board) == 0:
		...
		res = "<table border='1'>"
		res += "<tr><th>Rank</th><th>Time</th><th>Run ID</th><th>昵称</th><th>提交时间</th><th>评测时间</th></tr>"
		row_id = 0
		for row in board:
			row_id += 1
			res += "<tr>"
			res += "<td>%d</td>" % row_id
			res += "<td>%s</td>" % utils.render_time_ms(row[0])
