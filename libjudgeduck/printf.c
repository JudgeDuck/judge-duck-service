...
static void
jd_putch(int ch, struct printbuf *b)
{
	b->buf[b->idx++] = ch;
	if (b->idx == 256-1) {
...
int
jd_vcprintf(const char *fmt, va_list ap)
{
	struct printbuf b;

	b.idx = 0;
	b.cnt = 0;
	jd_vprintfmt((void*)jd_putch, &b, fmt, ap);
	sys_cputs(b.buf, b.idx);
...
int
jd_cprintf(const char *fmt, ...)
{
	va_list ap;
	int cnt;

	va_start(ap, fmt);
	cnt = jd_vcprintf(fmt, ap);
	va_end(ap);
